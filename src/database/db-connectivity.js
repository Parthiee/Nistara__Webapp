...
console.log(env);
const ASTRA_BASE_URL = `https://${env.ASTRA_DB_ID}-${env.ASTRA_DB_REGION}.apps.astra.datastax.com`;
    
export default class dbClient {
  constructor() {
    this.baseUrl = `${ASTRA_BASE_URL}/api/rest/v2/keyspaces/${env.ASTRA_DB_KEYSPACE}`;
    this.headers = {
  ...
  async getPosts() {
    try {
      const response = await axios.get(`${this.baseUrl}/posts/rows`, { headers: this.headers });
  ...
  async getRequestPosts() {
    try {
      const response = await axios.get(`${this.baseUrl}/requests/rows`, { headers: this.headers });
      const data = response.data.data;
